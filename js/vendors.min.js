...
let btn = document.querySelector('.start-dysplay__btn')
let startDysplay = document.querySelector('.start-dysplay')
let mainDysplay = document.querySelector('.main-dysplay')
let inputNameUser = document.querySelector('#name')
let check = document.getElementsByClassName('task-todo-main-dysplay__check')
let label = document.getElementsByClassName('task-todo-main-dysplay__label')
let tasktsHtml = document.querySelector('._taskts')

let curentTime = ("0" + new Date().getDate()).slice(-2) + '.' + ("0" + (new Date().getMonth() + 1)).slice(-2) + '.' + new Date().getFullYear()

btn.addEventListener('click', getName)

function animationStartBlock() {
	startDysplay.classList.toggle('_noactive')
	mainDysplay.classList.toggle('_active')
}


let namee = 'undefined'
// let namee = (namee == '') ? localStorage.name :


function getName() {
	if (inputNameUser.value == '' && namee == '') {

		document.querySelector('._name-error').innerHTML = 'давай знакомиться :)'

	} else {
		if (inputNameUser.value != '') {
			namee = inputNameUser.value;
		}
		document.querySelector('._name').innerHTML = namee;
		animationStartBlock()
		document.querySelector('._name-error').innerHTML = ''
	}
}

let tasksAdd = document.querySelector('.todo-main-dysplay__btn-add')
let popup = document.querySelector('.popup')

tasksAdd.addEventListener('click', showPopup)
...
let date
function getTasks() {
	document.querySelector('.ok').addEventListener('click', qqqq)
}

function qqqq() {
	tasks = document.querySelector('#task').value
	date = document.querySelector('._date').value

	if (tasks == '') {
		document.querySelector('.popup__error').innerHTML = 'дабавте задачу'
	} else {
		if (date !== 'date') {
			popup.classList.remove('show')
			showTasks()
		} else {
			document.querySelector('.popup__error').innerHTML = 'дабавте дату выполнения задачи'
		}
	}

}

let todoItems = document.querySelector('.todo-main-dysplay__items')
let id = 1;
function showTasks() {
	todoItems.insertAdjacentHTML('beforeend', `	<div class="todo-main-dysplay__task task-todo-main-dysplay">
								<div class="task-todo-main-dysplay__trash">
									<img src="img/trash.png" alt="">
								</div>
								<input type="checkbox" id='${id}' class="task-todo-main-dysplay__check">
								<label for="${id}" class="task-todo-main-dysplay__label"> ${tasks} </label>
								<div class="task-todo-main-dysplay__date">${date}</div>
							</div> `)
	id++;
	getTasksNumber()
}

let trash = document.getElementsByClassName('task-todo-main-dysplay__trash')
let todoMain = document.querySelector('.todo-main-dysplay__items')
todoMain.addEventListener('click', function (e) {
	e = e.target
	...
	for (const key in check) {
		if (Object.hasOwnProperty.call(check, key)) {
			...
			if (element.checked) {
				label[key].classList.add('_comptited')
				getTasksNumber()
			} else {
				label[key].classList.remove('_comptited')
				getTasksNumber()
			}
	...
	for (const key in trash) {
		if (Object.hasOwnProperty.call(trash, key)) {
			const element = trash[key];

			element.addEventListener('click', function () {
				let el = element.closest('.task-todo-main-dysplay')
...
function getTasksNumber() {
	let tasktsCheced = document.getElementsByClassName('_comptited').length
	console.log(tasktsCheced)
	let taskts = label.length - tasktsCheced
	tasktsHtml.innerHTML = taskts

}

let btnSort = document.querySelector('.todo-main-dysplay__btn-sort')

btnSort.addEventListener('click', sortTasks)

function sortTasks() {
	btnSort.classList.toggle('_active')
	if (btnSort.classList.contains('_active')) {
		btnSort.innerHTML = 'Показать все задачи'
		let dates = document.querySelectorAll('.task-todo-main-dysplay__date')
		dates.forEach(element => {
			console.log(element.textContent)
			if (element.textContent !== curentTime) {
				element.closest('.todo-main-dysplay__task').classList.add('_none')
				console.log('hi')
			}
		});

	} else {
		btnSort.innerHTML = 'Показать задачи на сегодня'
		document.querySelectorAll('.todo-main-dysplay__task').forEach(element => {
...
window.onunload = function () {
	...
	let task = document.querySelectorAll('.todo-main-dysplay__task')
...
function ergew() {
	for (let key in localStorage) {
		...
		todoItems.insertAdjacentHTML('beforeend', `	${localStorage[key]} `)
		id++;
		getTasksNumber()
		if (key != 'name') {
...
// console.log(namee != '' && namee != 'undefined')
// console.log(namee === undefined)
if (namee != '' && namee != 'undefined') {
	inputNameUser.remove()
	document.querySelector('._name-hello').innerHTML = `Привет ${namee}`
}

var timeNode = document.querySelector('.head-main-dysplay__logout');

setInterval(
	() => timeNode.innerHTML = new Date().toLocaleString("ru", options).replace(',', '<br/>').replace('г.', ''),
	1000
);
...
let height = document.querySelector('.main-dysplay__head').clientHeight + document.querySelector('.todo-main-dysplay__btn-add').clientHeight + document.querySelector('.todo-main-dysplay__btn-sort').clientHeight + 35 + 20 + 30
console.log(height)
document.querySelector('.todo-main-dysplay__items').style.height = 'calc(100vh - ' + height + 'px'
